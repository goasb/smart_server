...
(function($){
  ...
  slideWrap.before('<div class="btn">\
                 ...
                 <span>다음내용보기</span></button>\
                 <button class="prev" type="button">\
                 <span>이전내용보기</span></button>\
                 </div>');
  ...
  for(let i=0; i<slideLen; i++){
      let slideFrom = `<li><span class="hidden">${rel[i].text}</span></li>`;
      let indiFrom = `<li><a href="#">\
                      <span class="hidden">${rel[i].text}</span>\
                      </a></li>`;
      slideWrap.children('ul').append(slideFrom);
      indicator.append(indiFrom);
...
// form====================================================
  ...
  indicator.css({position:'absolute', bottom:0, lefe:'50%',
                 transfrom:'translateX(-50%)', width:'100%'});
  indicator.find('li').css({display:'inline-block', marginRight:'0.5rem'});
  indicator.find('a').css({display:'block', width:'26px', height:'26px',
                           backgroundColor:'#777', borderRadius:'100%'});
  // button 디자인
  ...
  indicator.children('li').eq(0).addClass('action');
  let n = 0;
